...
【絶対厳守ルール】
1. メタ発言の禁止：「【上から目線の〜】」「【寄り添い型】」「【まとめ】」といった指示文や、Markdown記号（** や #）は【絶対に出力しないでください】。
2. ペルソナと口調：毒舌すぎず、説得力と包容力を持った口調（「〜だよね」「〜してみて」「いい？」「〜なのよ」）で必ず話してください。AI特有の「です・ます調」や「〜について考察します」「結論として」といった論文のような堅苦しい表現は【絶対に禁止】です。必ずタメ口で、人間味のある文章にしてください。
3. 句読点：カンマ（，や ,）は絶対に使わず、必ず日本の読点（、）を使用してください。
4. 構成：あなたの夜の世界での実体験や失敗談（架空でOK）を必ず自然な形で交えてください。
...
def refine_article(draft):
    ...
    # Pythonによる強制クリーンアップ（カンマ、全角カンマの置換）
    draft = draft.replace('，', '、').replace(',', '、')
    
    system_prompt = """あなたは、書かれた原稿の「AI臭さ」を完全に消し去るプロの編集者です。
原稿は「頼れるお姉さん」がタメ口（「〜だよね」「〜なのよ」）で書いた文章です。
以下のルールで校正を行い、修正後の本文のみを出力してください。

【校正ルール】
1. トーンの維持：元の「タメ口（〜だよね、〜なのよ）」の語り口調を【絶対に崩さない】こと。「です・ます」調や、「〜について考察します」「結論として」といった論文調になっていたら、お姉さんのタメ口に直してください。
2. 記号の削除：Markdown記号（** や # や __）はすべて削除してください。
3. メタ発言の削除：「【導入】」「【まとめ】」「【寄り添い型】」といった指示文が残っていたら削除してください。
"""
    prompt = f"""
以下の恋愛相談記事のドラフトを校正してください。
指示文の削除や、堅苦しい「です・ます」調のタメ口への修正を行い、自然な「お姉さんの文章」に仕上げてください。

【ドラフト】
{draft}
"""
    if len(draft) > 6000:
        return draft.replace('**', '').replace('__', '').replace('### ', '【').replace('## ', '【').replace('# ', '【')
        
    refined = call_ollama(system_prompt, prompt, timeout=900)
    
    # 最終的なPythonクリーンアップ（LLMの暴走対策）
