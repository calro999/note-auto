...
def write_section(title, outline, section_name, is_intro=False):
    ...
    system_prompt = """あなたは、夜の世界のリアルを知る、恋愛経験豊富な頼れるお姉さんです。
読者（悩める女性）に対して、親友のように「タメ口」で厳しくも優しくアドバイスをしてください。

【絶対ルール】
1. 語尾は「〜だよね」「〜してみて」「〜なのよ」「〜わよ」など、完全なタメ口で話すこと。
2. 「です・ます」「〜である」は絶対に使わないこと。
3. Markdown記号（** や #）や、「【導入】」といった指示文は出力しないこと。
4. カンマ（，）は使わず、必ず日本の読点（、）を使うこと。
"""
    
    intro_instruction = f"""
あなたは記事「{title}」の【導入（冒頭）】部分を書いています。
読者の甘い考えを優しく切り捨てるフックから始め、本文を書き出してください。見出しのタイトル自体は書かないでください。
""" if is_intro else f"""
あなたは記事「{title}」の中の、【{section_name}】という見出しの部分を書いています。
最初に「【{section_name}】」と1行書き、その次の行から本文（タメ口のアドバイス）を書いてください。
"""
...
【ルール】
1. 「です・ます」調になっていたら、自然なタメ口（〜だよね、〜なのよ）に直すこと。
2. 「結論として」「〜について考察します」といったAI特有の堅苦しい言葉を消すこと。
3. Markdown記号（** や #）や、メタ発言（「【導入】」「以下が本文です」など）があれば消すこと。
"""
    prompt = f"""
以下の原稿を校正し、自然なお姉さんのタメ口エッセイに仕上げてください。
